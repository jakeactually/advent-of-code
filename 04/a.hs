...
main = do
    ...
    let (guardId, moments) = maximumBy (compare `on` length . snd) guardDatas
    let groupedByMinute = group $ sort $ moments
    let maxMinute = head $ maximumBy (compare `on` length) $ groupedByMinute
    print $ (guardId, maxMinute)
