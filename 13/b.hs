...
import Data.Map as M (Map, (!?), assocs, fromList, insert, delete, size)
...
tick tracks state (wagon@((y, x), (c, d)) : wagons) =
    ...
    in
        case state !? newPos of
            Just hitWagon ->
                let
                    newState = M.delete newPos $ M.delete (y, x) state
                    newWagons = L.delete (newPos, hitWagon) wagons
                in
                    tick tracks newState newWagons
            Nothing ->
                let
                    track = tracks ! newPos
                    ...
                    newState = insert newPos newWagon $ M.delete (y, x) state
                in
                    tick tracks newState wagons
tick tracks state [] = if size state < 2
    then fst $ head $ M.assocs state
    else tick tracks state $ M.assocs state
